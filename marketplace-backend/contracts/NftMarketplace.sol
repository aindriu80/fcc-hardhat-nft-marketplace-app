...
error NFT_MARKETPLACE__NoProceeds();
error wFT_MARKETPLACE__TransferFailed();
...
contract NftMarketplace is ReentrancyGuard {
    ...
    function listItem(
        ...
        // need to use Chainlink Price Feeds to conver thte price of the tokens between each other
        ...
        isListed(nftAddress, tokenId)
    {
        ...
        // Check out https://fravoll.github.io/solidity-patterns/pull_over_push.html

        // We don't want to send the money to the user ...❌
        // They have to withdraw the money ✅
    ...
    function withdrawProceeds() external {
        ...
        if (proceeds <= 0) {
            revert NFT_MARKETPLACE__NoProceeds();
        }
        s_proceeds[msg.sender] = 0;
        (bool success, ) = payable(msg.sender).call{value: proceeds}("");
        if (!success) {
            revert wFT_MARKETPLACE__TransferFailed();
        }
    }
...
// 1. `listItem`: List NFTs on the marketplace ✔️
// 2. `buyItem`: Buy the NFTs ✔️
// 3. `cancelItem`: Cancel a listing ✔️
// 4. `updateListing`: Update Price ✔️
// 5. `withdrawProceeds`: Withdraw payment for my bought NFTs ✔️
